...
def test_deb(o):
    with docker('odoo-%s-debian-nightly-tests' % version, o.build_dir, o.pub) as wheezy:
        ...
        wheezy.system('su odoo -s /bin/bash -c "odoo.py -c /etc/odoo/shmj-server.conf -d mycompany -i base --stop-after-init"')
        wheezy.system('su odoo -s /bin/bash -c "odoo.py -c /etc/odoo/shmj-server.conf -d mycompany &"')
...
def test_rpm(o):
    with docker('odoo-%s-centos-nightly-tests' % version, o.build_dir, o.pub) as centos7:
        ...
        centos7.system('su odoo -s /bin/bash -c "openerp-server -c /etc/odoo/shmj-server.conf -d mycompany -i base --stop-after-init"')
        centos7.system('su odoo -s /bin/bash -c "openerp-server -c /etc/odoo/shmj-server.conf -d mycompany &"')
